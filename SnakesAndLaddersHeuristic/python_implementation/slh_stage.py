...
def stage0():
    ...
    while slh_transform.closing_transformation(solver.ordering_stack[-1]): pass
    solver.n_gaps = solver.ordering_stack[-1].n_gaps()
    solver.bar.update(slh_util.ordering.size-solver.n_gaps)
...
def stage1():
    ...
    while len(solver.ordering_stack)<slh_util.ordering.size**2 and aux_ordering_stack:
        ...
        if n_ordr:
            ...
            if opt:
                n_gaps -= opt
                solver.bar.update(slh_util.ordering.size-n_gaps)
                if n_gaps==0:
                    solver.n_gaps = 0
                    return
    ...
    while len(solver.ordering_stack)<=slh_util.ordering.size**3:
        ...
        while len(solver.ordering_stack)<=slh_util.ordering.size**3 and aux_ordr_pair_stack:
            ...
            if n_ordr:
                ...
                solver.bar.update(slh_util.ordering.size-aux_ordr_pair_stack[-1][0])
