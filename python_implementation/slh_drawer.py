...
class slh_drawer(object):
    ...
    def draw(self,ordering,background_color=None,layer=None):
        if layer == "snakes": image = self.draw_snakes(ordering)
        elif layer == "ladders": image = self.draw_ladders(ordering)
        else: image = Image.alpha_composite(self.draw_ladders(ordering),self.draw_snakes(ordering))
